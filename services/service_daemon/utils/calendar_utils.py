...
def get_upcoming_events():
    ...
    try:
        ...
        cursor.execute("SELECT title, start_time, address, notes FROM events WHERE start_time BETWEEN %s AND %s AND address IS NOT NULL AND address != '' ORDER BY start_time ASC LIMIT 1", (now, future))
        row = cursor.fetchone()
        db.close()
        if row:
            return [{'title': row[0], 'start_time': row[1], 'address': row[2], 'notes': row[3]}]
